  ...
  export default {
    ...
    methods: {
      ...
      infinite() {
        ...
        that.disabled = true;
        that.getSelfList(++that.pageInfo.current_page, that.loading.last_id).then(() => {
          // 触底判断
          that.disabled = false;
      ...
      // 重置Task列表
      resetList(last_id, currPage, qtype = this.active_task, qdep_id = this.active_part[0], quser_id = this.active_part[1], okr_type = this.active_kind[0]) {
        let that = this;
        that.okr_list = [];
        ...
        TaskApi().getTaskList({last_id, currPage, qtype, qdep_id, quser_id, okr_type}).then(res => {
