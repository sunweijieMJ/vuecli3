...
<script>
  ...
  export default {
    ...
    methods: {
      // 想法列表
      async getIdeaList(curPage, last_id) {
        let that = this;
        await IdeaApi().getIdeaList({curPage, last_id}).then(res => {
      ...
      infinite() {
        ...
        that.disabled = true;
        that.getIdeaList(++that.pageInfo.current_page, that.loading.last_id).then(() => {
          // 触底判断
          that.disabled = false;
