<template>
  <div class="task-check" v-if="item.status === 1 && (item.task_owner_id === self_info.user_id || (item.participants ? item.participants.indexOf(self_info.user_id) !== -1 : false))">
    <li v-for="(menuitem, index) in check_list" :key="index" @click.stop="handleTask(menuitem)"
      v-if="(item.is_key_task && item.participants.indexOf(self_info.user_id) !== -1 && menuitem.name === '添加Task') || (item.task_owner_id === self_info.user_id && menuitem.name !== '添加Task')">
