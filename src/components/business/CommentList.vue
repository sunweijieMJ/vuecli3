<template>
  <ul class="comment-list">
    <li v-for="(vitem, vindex) in list" :key="vindex" :class="{second: !vitem.replys.length}">
      <single-comment :item="vitem" :root="{comment_id: vitem.comment_id}" @commentSuccess="commentSuccess" @deleteSuccess="deleteSuccess"></single-comment>
      <ul class="second-list" v-if="vitem.replys && vitem.replys.length">
        <li v-for="(witem, windex) in (show_more[vindex] ? vitem.replys : vitem.replys.slice(0, 2))" :key="windex">
          <single-comment :item="witem" :root="{list, comment_id: vitem.comment_id, index: windex}" @commentSuccess="commentSuccess" @deleteSuccess="deleteSuccess"></single-comment>
        </li>
